...
## Daytona CLI helper (TypeScript)

- Source: `scripts/daytona-cli.ts` (creates sandbox, push/pull files via base64 chunks, run commands, optional `--keep`).
- Example run (requires `DAYTONA_API_KEY` env and Bun):
  ```
  bun run scripts/daytona-cli.ts -- --push ./local.txt --cmd "cat /tmp/daytona-cli/local.txt" --pull /tmp/daytona-cli/local.txt:./local.out
  ```
- Options:
  - `--push local[:remote]` (repeatable), `--pull remote[:local]` (repeatable)
  - `--cmd "<command>"`, `--language <lang>` (default `typescript`)
  - `--remote-dir <dir>` base for relative paths (default `/tmp/daytona-cli`)
  - `--keep` leaves sandbox alive

## Supporting files

- `test/daytona-cli-example.sh`: smoke script living under `test/` to exercise the CLI (creates a sample file, pushes, runs `wc -c`, pulls back).
- `plan.md`: checklist tracking the Daytona CLI work items.
